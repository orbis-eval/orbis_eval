...
def write_settings(user_dir, settings_dict):
    with open('orbis_eval/config/settings.json', 'r', encoding='utf-8') as settings_file:
        settings = json.load(settings_file)

    settings['user_dir'] = str(user_dir)
    settings.update(settings)
    with open('orbis_eval/config/settings.json', 'w', encoding='utf-8') as settings_file:
