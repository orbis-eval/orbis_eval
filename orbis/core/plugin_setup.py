...
class PluginSetupBaseClass(object):
    """docstring for PluginSetupBaseClass"""

    def __init__(self):
        super(PluginSetupBaseClass, self).__init__()

    def run_additional_setup(self, plugin_name):
        add_setup = False
        try:
            add_setup = importlib.import_module(f"{plugin_name}.additional_setup")
    ...
    def run(self, directory):
        ...
        description = f"{parts[3].capitalize()} {parts[2].capitalize()} {parts[1].capitalize()} for {parts[0].capitalize()}"
...
if __name__ == '__main__':
    ...
    PluginSetupBaseClass().run(directory)
