...
class OrbisSetupBaseClass(object):
    ...
    def load_requirements_file(self, plugin_name, metadata, dev):
        requirements = ["orbis"] if metadata["type"] != "main" else []
    ...
    def parse_metadata(target, file_content):
        return re.search(r"^{target} = ['\"]([^'\"]*)['\"]", file_content, re.MULTILINE).group(1)
    ...
    def load_metadata(self, plugin_name):
        metadata = {}

        with io.OpenWrapper(f"{plugin_name}/__init__.py", "rt", encoding="utf8") as open_file:
